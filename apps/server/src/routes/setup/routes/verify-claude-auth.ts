  ...
  return async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      try {
        ...
        if (authMethod === 'cli') {
          // Check if using custom API endpoint (e.g., GLM proxy)
          // Custom endpoints require API key even for "CLI" mode
          const usesCustomEndpoint = !!process.env.ANTHROPIC_BASE_URL;

          if (usesCustomEndpoint) {
            // For custom endpoints, keep the API key/auth token
            logger.info('[Setup] Using custom API endpoint - keeping API key for CLI verification');
          } else {
        ...
        // Only clear keys if not using custom endpoint
        if (authMethod === 'cli') {
          const usesCustomEndpoint = !!process.env.ANTHROPIC_BASE_URL;
          if (!usesCustomEndpoint) {
            if (originalAnthropicKey === undefined) {
